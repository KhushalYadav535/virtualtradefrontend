...
export const portfolio = {
  ...
  exportHoldingsReport: async () => {
    const token = typeof globalThis !== 'undefined' ? globalThis.token : null;
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/portfolio/holdings/export/report`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (!res.ok) throw new Error('Export failed');
    const blob = await res.blob();
  ...
  exportHoldingsPdf: async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/portfolio/holdings/export/report?print=1`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (!res.ok) throw new Error('Export failed');
  ...
  exportTradesCsv: async (params = {}) => {
    ...
    const url = `${process.env.NEXT_PUBLIC_API_URL}/portfolio/trades/export${qs ? `?${qs}` : ''}`;
  ...
  exportTradeBookTax: async (params = {}) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/portfolio/trades/export/report?${qs}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
