...
export const isApiTimeout = (err) =>
  err?.code === 'ECONNABORTED' || /timeout/i.test(err?.message || '');

export const isRetryableNetworkError = (err) =>
  !err?.response && (isApiTimeout(err) || err?.code === 'ERR_NETWORK');

function healthUrl() {
  return `${getApiBaseUrl().replace(/\/api\/?$/, '')}/health`;
}

/** Single ping — wake Render free tier (no auth). */
export function warmBackend() {
  const url = healthUrl();
  if (!url.includes('http')) return Promise.resolve(false);
  const ctrl = typeof AbortController !== 'undefined' ? new AbortController() : null;
  const timer = ctrl && setTimeout(() => ctrl.abort(), 5000);
...
/** Poll /health until backend responds or maxMs elapsed. */
export async function waitForBackend(maxMs = 12000) {
  const start = Date.now();
  while (Date.now() - start < maxMs) {
    if (await warmBackend()) return true;
    await delay(1500);
  }
  return false;
}
...
api.interceptors.request.use((config) => {
  const token = (typeof window !== 'undefined' && localStorage.getItem('token')) || (typeof globalThis !== 'undefined' ? globalThis.token : null);
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (!config.__skipRetry && isRetryableNetworkError(error) && method === 'get') {
      config.__retryCount = (config.__retryCount || 0) + 1;
      if (config.__retryCount <= 2) {
        await warmBackend();
        await delay(1500 * config.__retryCount);
        return api.request({ ...config, __skipRetry: config.__retryCount >= 2 });
      }
    }
...
export const auth = {
  ...
  resetInitialPassword: (data) => api.post('/auth/password/reset-initial', data)
};
...
export const admin = {
  ...
  addFunds: (studentId, amount) => api.post(`/admin/students/${studentId}/add-funds`, { amount }),
  getCustomMarketData: () => api.get('/admin/custom-market-data'),
  updateCustomMarketData: (data) => api.post('/admin/custom-market-data', data)
};
